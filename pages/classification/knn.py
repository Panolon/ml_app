...
def run():
    ...
    if uploaded_file:
        ...
        n_neighbors = st.sidebar.slider("Number of Neighbors (K)", min_value=1, max_value=int(len(X_train)/2), value=5, step=1)
        weights = st.sidebar.selectbox("Weights", options=["uniform", "distance"], index=1)
        algorithm = st.sidebar.selectbox("Algorithm", options=["auto","ball_tree","kd_tree","brute"],index=0)
        power = st.sidebar.slider("L_p distance",min_value=1, max_value=10, step=1, value=2)
        ...
        precisions, recalls, thresholds1 = precision_recall_curve(y_test, y_proba[:, 1])  #precision - racall curves 
        fpr, tpr, thresholds = roc_curve(y_test, y_proba[:, 1])  # roc curve FPR / TPR
        certainty = abs(y_proba[:, 1] - y_proba[:, 0]) # certainty plot
